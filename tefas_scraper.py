...
Kullanim:
  py tefas_scraper.py                  # tum HSYF, 5 yil, data/
...
def write_manifest(data_dir: Path, entries: List[Dict[str, Any]], hedef_gun: int) -> None:
    manifest = {
...
def refresh_manifest_from_disk(data_dir: Path, hedef_gun: int) -> None:
    ...
    write_manifest(data_dir, entries, hedef_gun)
...
def run_single_fund(
    ...
    refresh_manifest_from_disk(data_dir, gun)
...
def run_full_hsyf(
    ...
    write_manifest(data_dir, entries, gun)
...
def main() -> int:
    ...
    if args.manifest_yenile:
        refresh_manifest_from_disk(data_dir, args.gun)
